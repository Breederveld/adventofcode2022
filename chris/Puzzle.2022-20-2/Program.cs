...
namespace Puzzle_2022_20_2
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var ints = strings.Where(st => !string.IsNullOrWhiteSpace(st)).Select(st => long.Parse(st)).ToList();
            var mul = 811589153;
            ints = ints.Select(i => i * mul).ToList();
            sw.Start();
            ...
            for (var round = 0; round < 10; round++)
            {
                for (var iidx = 0; iidx < ints.Count; iidx++)
                {
                    ...
                    var newIdx = (int)(((idx + i) % (ints.Count - 1) + (ints.Count - 1)) % (ints.Count - 1));
                    if (idx < newIdx)
                    {
                        newIdx++;
                    }
